...
class LrnLayer private constructor(nativeObjectHandle: Long) : LayerBase(nativeObjectHandle) {
     ...
     **/
    @UserConstructor(R2.string.layer_lrn, 0)
    constructor(@UserField(UserFields.InWidth) in_width: Long,
                @UserField(UserFields.InHeight) in_height: Long,
                @CustomUserField(R2.string.local_size, R2.string.local_size_hint, InputType.TYPE_CLASS_NUMBER) local_size: Long,
                @CustomUserField(R2.string.alpha, R2.string.alpha_hint, InputType.TYPE_NUMBER_FLAG_DECIMAL) alpha: Double,
                @CustomUserField(R2.string.beta, R2.string.beta_hint, InputType.TYPE_NUMBER_FLAG_DECIMAL) beta: Double) :
