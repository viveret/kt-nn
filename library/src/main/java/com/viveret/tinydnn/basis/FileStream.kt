...
class FileStream: Stream {
    ...
    constructor(namespace: String, name: String, mime: String, extension: String, id: UUID, host: Host? = null, protocol: String = "", args: String = "", remotePath: String? = null) {
        this.id = id
        this.name = name
    ...
    constructor(file: File, id: UUID, host: Host? = null, protocol: String = "", args: String = "", remotePath: String? = null):
            this(file.parent, file.nameWithoutExtension, extensionToMime(file.extension), ".${file.name.substringAfter(".")}", id, host, protocol, args, remotePath)

    constructor(path: String, id: UUID):
            this(File(path), id)

    constructor(url: URL, id: UUID, host: Host?):
            this(File(url.path), id, host, protocol = url.protocol, args = url.query ?: "", remotePath = url.path)

    constructor(fileName: String, id: UUID, url: URL? = null, host: Host? = null) {
        this.id = id
        this.host = host
        try {
    ...
    override fun sourceStream(source: DataSource, context: Context): InputStream {
        return when (source) {
            ...
            DataSource.LocalFile -> {
                val suiteDir = File(context.cacheDir, this.host!!.id.toString())
                if (suiteDir.isDirectory || suiteDir.mkdirs()) {
                    val f = File(suiteDir, this.id.toString())
    ...
    override fun isAvailable(source: DataSource, context: Context): Boolean {
        return when (source) {
            DataSource.LocalFile -> {
                val suiteDir = File(context.cacheDir, this.host!!.id.toString())
                val f = File(suiteDir, this.id.toString())
                return suiteDir.exists() && suiteDir.isDirectory && f.isFile && f.canRead()
            }
    ...
    override fun equals(other: Any?): Boolean =
            other != null && other is Stream && (this === other || this.name == other.name && this.extension == other.extension)

    override fun hashCode(): Int = (131 + name.hashCode() * 31) + extension.hashCode()
